#!/bin/bash
size=1				#Размер удаляемых файлов
maxdays=3			#Количество дней
home=/media/backup/backup	#Где чистим ...

echo ""
echo `date +%c` "### запущен скрипт удаления файлов ###"
...
echo "Всего директорий "$path
echo "Должно остаться "$maxdays
