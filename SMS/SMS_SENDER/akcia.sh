...
echo "select id,sot_tel,last_time from $base.$table where send = '0' ORDER BY id LIMIT $sms_num" > $MY_PATH/sql
...
echo "ДЛЯ ПРЕРЫВАНИЯ НАЖМИ CTRL+C"
        test_while=0
        while [ "$test_while" -ne "1" ]
        do
                read -p "(Y/N)" answer
